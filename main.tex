...
\end{itemize}
\algo{Guter Stil}: Feature based Branches, kleine Commits, aussagekräftige Commit-Nachrichten.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \lstinline|float b = 7/2;| $\implies$ Ganzzahlige Division, Ergebnis 3 wird zu float (3.0)
...
\begin{itemize}
   ...
   \item \datastruct{Konstruktor / Destruktor}: Zuerst wird der Konstruktor der Basisklasse (Elternklasse) aufgerufen, danach der Konstruktor der abgeleiteten Klasse (Kindklasse). Destruktoren werden in exakt umgekehrter Reihenfolge aufgerufen
...
\end{itemize}
\algo{Liskovsches Substitutionsprinzip}: Besagt, dass Objekte einer Unterklasse in der Lage sein müssen, Objekte ihrer Oberklasse zu ersetzen, ohne die korrekte Funktionsweise eines Programms zu beeinträchtigen.
...
\begin{itemize}
    ...
    \item \lstinline|auto p = std::make_shared<T>(arg1, arg2);| (mit Konstruktorargumenten)
    \begin{itemize}
        \item Objekt direkt übergeben: \lstinline|std::make_shared<T>(T(...))|
    \end{itemize}
\end{itemize}
...
\datastruct{Resource Acquisition Is Initialization}: Die Ressource wird im Konstruktor eines Objekts erworben ("Acquisition/Initialization") und im Destruktor automatisch wieder freigegeben ($\implies$ Prinzip hinter Smart Pointern).
...
\bottomrule
\end{tabularx}
Einfach verkettet: nur vorwärts; doppelt verkettet: vor- und rückwärts, mehr Speicher.
\section{Rechnerarchitektur}
\begin{itemize}
\item \textbf{Harvard-Architektur}: Getrennte Speicher für Daten und Befehle, paralleler Zugriff möglich.
\item textbf{Von Neumann Flaschenhals}: Gemeinsamer Bus für Daten und Befehle, begrenzt die Geschwindigkeit.
\item \textbf{CISC}: Viele komplexe Befehle, Hardware aufwendig, langsamer. 
\item \textbf{RISC}: Wenige, einfache Befehle, Hardware einfach, schneller, Pipelining möglich.
\item \textbf{Pipelining}: Überlappende Ausführung von Befehlen in verschiedenen Stadien des Zyklus.
\end{itemize}
...
\algo{Latenzzeit}: Zeit vom Eintreten eines Ereignisses bis zur Reaktion der Steuerung.
\algo{Eingangsverzögerung}: Zeit vom Eintreten eines Ereignisses bis zum Einlesen der Eingänge.
