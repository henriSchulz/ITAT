...
\begin{tabularx}{\linewidth}{l >{\RaggedRight}X}
\toprule
\textbf{Beziehung} & \textbf{Beschreibung \& C++ Implementierung} \\
\midrule
\textbf{Assoziation} & 
("kennt-ein") Objekte sind unabhängig. \newline
\textbf{C++:} Roher Zeiger (\texttt{*}) oder Referenz (\texttt{\&}), da keine Besitzübernahme. \newline
\textbf{Bsp:} \texttt{class Kurs; class Student \{ Kurs* k; \};} \\
\addlinespace % Fügt etwas Platz hinzu
\textbf{Aggregation} & 
("hat-ein") Teil-Ganzes, Lebensdauer der Teile ist \textbf{unabhängig}. \newline
\textbf{C++:} \texttt{std::shared\_ptr}, um geteilten Besitz darzustellen. \newline
\textbf{Bsp:} \texttt{class Team \{ std::vector<std::shared\_ptr<Spieler>> p; \};} \\
\addlinespace
\textbf{Komposition} & 
("enthält-ein") Teil-Ganzes, Lebensdauer des Teils ist \textbf{abhängig}. \newline
\textbf{C++:} \texttt{std::unique\_ptr} für exklusiven Besitz oder direktes Member-Objekt. \newline
\textbf{Bsp:} \texttt{class Haus \{ std::unique\_ptr<Zimmer> z; \};} \\
\addlinespace
\textbf{Vererbung} & 
("ist-ein") Eine Klasse erbt von einer anderen. \newline
\textbf{C++:} \texttt{class Student : public Person \{ ... \};} \\
\bottomrule
\end{tabularx}
